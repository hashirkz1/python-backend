    ...
    return (
      <div className='container'>
        {([audioPath1, audioPath2]).map((path, index) => (
          <div key={index}>
            <audio ref={audioRefs[index]} controls>
              <source src={path} type="audio/wav" />
              Your browser does not support the audio tag.
            </audio>
  
            <div className='my-2 container-sm'>
              <button style={{marginRight:"10vw", borderRadius:'1.5rem', padding:'0.2rem 1.5rem', border:'solid 2px #2B6CB0', background:'transparent'}} onClick={() => togglePlayPause(index)}>
